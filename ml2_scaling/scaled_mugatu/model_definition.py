...
def create_walker_urdf(scale=1, ground_friction=0.5, feet_friction=0.3): 
    ...
    def s_mass(m):  # scale mass
        return str((scale ** 2) * float(m))

    def s_inertia(i):  # scale inertia
        return str((scale ** 4) * float(i))

    def mesh_res_hint(val):  # scale mesh resolution
        return str(scale * float(val))
    
    def fix(i): # the mass of the right leg is wrong in the CAD unfortunately
        return str(0.01257/0.009502*float(i))
...
<robot name="walker">
  ...
  <link name="ground">
      ...
      <geometry>
          ...
          <drake:mesh_resolution_hint value="{mesh_res_hint('0.00157')}"/>
          </drake:proximity_properties>
      </collision>
  </link>
  ...
  <link name="right_leg">
    <inertial>
      <origin xyz="{s('-0.0153442 -0.0121619 -0.0140678')}" rpy="0 0 0"/>
      <mass value="{s_mass('0.01096')}"/>
      <inertia ixx="{s_inertia('1.21969e-06')}" 
      ixy="{s_inertia('4.28942e-07')}" 
      ixz="{s_inertia('3.2085e-07')}" 
      iyy="{s_inertia('2.68236e-06')}" 
      iyz="{s_inertia('-1.29338e-09')}" 
      izz="{s_inertia('2.18451e-06')}"/>
    </inertial>

    <!-- Part handweight_nut -->
    <visual>
      <origin xyz="{s('-0.0376272 -0.00148644 -0.00783136')}" rpy="0.698132 2.1684e-19 -1.38778e-17"/>
      <geometry>
        <mesh filename="file://{_MESH_DIR}/handweight_nut.obj" scale = "{scale} {scale} {scale}"/>
      </geometry>
      <material name="handweight_nut_material">
    ...
    <!-- Part left_arm -->
    <visual>
      <origin xyz="{s('-0.00987722 -0.0153115 -0.00188286')}" rpy="-4.98733e-18 -0 -1.5708"/>
      <geometry>
        <mesh filename="file://{_MESH_DIR}/left_arm.obj" scale = "{scale} {scale} {scale}"/>
      </geometry>
    ...
    <!-- Part handweight_nut_2 -->
    <visual>
      <origin xyz="{s('-0.0376272 -0.00700196 -0.0124594')}" rpy="0.698132 -6.50521e-19 -1.38778e-17"/>
      <geometry>
        <mesh filename="file://{_MESH_DIR}/handweight_nut.obj" scale = "{scale} {scale} {scale}"/>
      </geometry>
      <material name="handweight_nut_2_material">
        <color rgba="0.8 0.8 0.8 1.0"/>
      </material>
    </visual>
    <!-- 
    ...
    <!-- Part right_sclmug_foot -->
    <visual>
      <origin xyz="{s('-0.0123272 -0.0132315 -0.00828286')}" rpy="0 -0 0"/>
      <geometry>
        <mesh filename="file://{_MESH_DIR}/right_sclmug_foot.obj" scale = "{scale} {scale} {scale}"/>
      </geometry>
      <material name="right_sclmug_foot_material">
    ...
    <visual>
      <origin xyz="{s('-0.0123272 -0.0132315 -0.00828286')}" rpy="0 -0 0"/>
      <geometry>
        <mesh filename="file://{_MESH_DIR}/right_leg_new.obj" scale = "{scale} {scale} {scale}"/>
      </geometry>
      <material name="right_leg_new_material">
        <color rgba="0.8 0.8 0.8 1.0"/>
      </material>
    </visual>
    <!-- 
    ...
    <!-- Part handweight_bolt -->
    <visual>
      <origin xyz="{s('-0.0376272 -0.00535496 -0.0110774')}" rpy="0.698132 -1.43115e-17 0"/>
      <geometry>
        <mesh filename="file://{_MESH_DIR}/handweight_bolt.obj" scale = "{scale} {scale} {scale}"/>
      </geometry>
      <material name="handweight_bolt_material">
        <color rgba="0.8 0.8 0.8 1.0"/>
      </material>
    </visual>
    <!-- 
    ...
    <!-- Part handweight_nut_3 -->
    <visual>
      <origin xyz="{s('-0.0376272 -0.00332494 -0.00937405')}" rpy="0.698132 2.1684e-19 -1.38778e-17"/>
      <geometry>
        <mesh filename="file://{_MESH_DIR}/handweight_nut.obj" scale = "{scale} {scale} {scale}"/>
      </geometry>
      <material name="handweight_nut_3_material">
        <color rgba="0.8 0.8 0.8 1.0"/>
      </material>
    </visual>
    <!-- 
    ...
    <!-- Part top_screw -->
    <visual>
      <origin xyz="{s('-0.0102272 -0.0153115 0.00400199')}" rpy="-1.5708 -1.56125e-17 -1.07427"/>
      <geometry>
        <mesh filename="file://{_MESH_DIR}/top_screw.obj" scale = "{scale} {scale} {scale}"/>
      </geometry>
      <material name="top_screw_material">
        <color rgba="0.8 0.8 0.8 1.0"/>
      </material>
    </visual>
    <!-- 
    ...
    <!-- Part handweight_nut_4 -->
    <visual>
      <origin xyz="{s('-0.0376272 -0.0103726 -0.0152877')}" rpy="0.698132 -1.40946e-17 0"/>
      <geometry>
        <mesh filename="file://{_MESH_DIR}/handweight_nut.obj" scale = "{scale} {scale} {scale}"/>
      </geometry>
      <material name="handweight_nut_4_material">
        <color rgba="0.8 0.8 0.8 1.0"/>
      </material>
    </visual>
    <!-- 
    ...
    <!-- Part handweight_nut_5 -->
    <visual>
      <origin xyz="{s('-0.0376272 -0.00516345 -0.0109167')}" rpy="0.698132 2.1684e-19 -1.38778e-17"/>
      <geometry>
        <mesh filename="file://{_MESH_DIR}/handweight_nut.obj" scale = "{scale} {scale} {scale}"/>
      </geometry>
      <material name="handweight_nut_5_material">
        <color rgba="0.8 0.8 0.8 1.0"/>
      </material>
    </visual>
    <!-- 
  ...
  <link name="right_hardstop">
    <inertial>
      <origin xyz="{s('3.69994e-19 0.00346118 0.001625')}" rpy="0 0 0"/>
    ...
    <!-- Part right_hardstop -->
    <visual>
      <origin xyz="{s('0.00223 0.00525 -0.001')}" rpy="1.5708 1.5708 0"/>
      <geometry>
  ...
  <joint name="r_hardstop_joint" type="fixed">
    <origin xyz="{s('-0.0133272 -0.0154615 -0.00303286')}" rpy="-1.5708 -0 1.5708"/>
  ...
  <link name="left_leg">
    <inertial>
      <origin xyz="{s('0.00334794 0.0116059 -0.00264927')}" rpy="0 0 0"/>
      <mass value="{s_mass('0.01075')}"/>
      <inertia ixx="{s_inertia('2.55956e-06')}" 
      ixy="{s_inertia('2.41711e-08')}" 
      ixz="{s_inertia('4.13792e-07')}" 
      iyy="{s_inertia('2.20926e-06')}" 
      iyz="{s_inertia('-4.06791e-07')}" 
      izz="{s_inertia('1.07892e-06')}"/>
    </inertial>
    
    <!-- Part left_leg -->
    <visual>
      <origin xyz="{s('0.00223 0.00525 0.00075')}" rpy="1.5708 1.5708 0"/>
      <geometry>
        <mesh filename="file://{_MESH_DIR}/left_leg_new.obj" scale = "{scale} {scale} {scale}"/>
      </geometry>
      <material name="left_leg_new_material">
        <color rgba="0.8 0.8 0.8 1.0"/>
      </material>
    </visual>
    <!-- 
    ...
    <!-- Part left_sclmug_foot -->
    <visual>
      <origin xyz="{s('0.00223 0.00525 0.00075')}" rpy="1.5708 1.5708 0"/>
      <geometry>
        <mesh filename="file://{_MESH_DIR}/left_sclmug_foot.obj" scale = "{scale} {scale} {scale}"/>
      </geometry>
      <material name="left_sclmug_foot_material">
        <color rgba="0.6 0.68 0.75 1.0"/>
      </material>
    </visual>
    <collision>
    ...
    <!-- Part handweight_bolt_2 -->
    <visual>
      <origin xyz="{s('0.010124 0.00783282 -0.0243')}" rpy="2.29002 1.5708 0"/>
      <geometry>
        <mesh filename="file://{_MESH_DIR}/handweight_bolt.obj" scale = "{scale} {scale} {scale}"/>
      </geometry>
      <material name="handweight_bolt_2_material">
        <color rgba="0.8 0.8 0.8 1.0"/>
      </material>
    </visual>
    <!-- 
    ...
    <!-- Part handweight_nut_6 -->
    <visual>
      <origin xyz="{s('0.00519627 0.012148 -0.0243')}" rpy="2.29002 1.5708 0"/>
      <geometry>
        <mesh filename="file://{_MESH_DIR}/handweight_nut.obj" scale = "{scale} {scale} {scale}"/>
      </geometry>
      <material name="handweight_nut_6_material">
        <color rgba="0.8 0.8 0.8 1.0"/>
      </material>
    </visual>
    <!-- 
    ...
    <!-- Part handweight_nut_7 -->
    <visual>
      <origin xyz="{s('0.010312 0.00766812 -0.0243')}" rpy="2.29002 1.5708 0"/>
      <geometry>
        <mesh filename="file://{_MESH_DIR}/handweight_nut.obj" scale = "{scale} {scale} {scale}"/>
      </geometry>
      <material name="handweight_nut_7_material">
        <color rgba="0.8 0.8 0.8 1.0"/>
      </material>
    </visual>
    <!-- 
    ...
    <!-- Part right_arm -->
    <visual>
      <origin xyz="{s('1.73472e-17 6.07153e-18 0.0007')}" rpy="1.5708 2.22827e-16 -0.02109"/>
      <geometry>
        <mesh filename="file://{_MESH_DIR}/right_arm.obj" scale = "{scale} {scale} {scale}"/>
      </geometry>
    ...
    <!-- Part handweight_nut_8 -->
    <visual>
      <origin xyz="{s('0.00850647 0.00924924 -0.0243')}" rpy="2.29002 1.5708 0"/>
      <geometry>
        <mesh filename="file://{_MESH_DIR}/handweight_nut.obj" scale = "{scale} {scale} {scale}"/>
      </geometry>
      <material name="handweight_nut_8_material">
        <color rgba="0.8 0.8 0.8 1.0"/>
      </material>
    </visual>
    <!-- 
    ...
    <!-- Part handweight_nut_9 -->
    <visual>
      <origin xyz="{s('0.0139232 0.00450588 -0.0243')}" rpy="2.29002 1.5708 0"/>
      <geometry>
        <mesh filename="file://{_MESH_DIR}/handweight_nut.obj" scale = "{scale} {scale} {scale}"/>
      </geometry>
      <material name="handweight_nut_9_material">
        <color rgba="0.8 0.8 0.8 1.0"/>
      </material>
    </visual>
    <!-- 
    ...
    <!-- Part handweight_nut_10 -->
    <visual>
      <origin xyz="{s('0.0121176 0.006087 -0.0243')}" rpy="2.29002 1.5708 0"/>
      <geometry>
        <mesh filename="file://{_MESH_DIR}/handweight_nut.obj" scale = "{scale} {scale} {scale}"/>
      </geometry>
      <material name="handweight_nut_10_material">
        <color rgba="0.8 0.8 0.8 1.0"/>
      </material>
    </visual>
    <!-- 
  ...
  <joint name="l_hardstop_a_joint" type="fixed">
    <origin xyz="{s('-1.73472e-18 1.73472e-18 0')}" rpy="3.14159 -0 0"/>
  ...
  <joint name="l_hardstop_b_joint" type="fixed">
    <origin xyz="{s('-1.73472e-18 1.73472e-18 0')}" rpy="3.14159 -0 0"/>
    <parent link="left_leg"/>
  ...
  <joint name="hip" type="revolute">
    <origin xyz="{s('-0.0173272 -0.0154615 -0.00303286')}" rpy="-1.5708 -0 1.5708"/>
    <parent link="right_leg"/>
    <child link="left_leg"/>
    <axis xyz="0 0 1"/>
...
def get_hip_torque(scale, safety_factor=1.0):
    SCALE_DATA = np.array([ 0.9,        1,           1.014,       1.03011523,  1.17904154, 
                           1.34949849,  1.54459883,  1.76790531,  2.02349576,  2.31603756, 
                           2.65087284,  3.03411610,  3.47276578,  3.97483212,  4.54948345,  
                           5.20721354,  5.96003330,  6,           6.283,       6.82169007,
                           7.80791870,  8.93672885,  10.22873387, 11.70752726, 13.40011348, 
                           15.33740107, 17.55476713, 20.09270330, 22.99755518, 26.32236869, 
                           30.12785873, 34.48351788, 39.46888547, 40,          40.486, 
                           41.068,      42,          45.175
                           ], dtype=float)
    
    TORQUE_DATA = np.array([
                            0.0014,      0.0025,     0.0025,      0.0025,      0.0028, 
                            0.0040,      0.0065,     0.0092,      0.015,       0.021, 
                            0.033,       0.045,      0.069,       0.095,       0.17, 
                            0.23,        0.33,       0.34,        0.39,        0.55, 
                            0.77,        1.0,        1.9,         3.0,         3.7, 
                            5.2,         7.5,        15,          25,          28, 
                            41,          54,         90,          95,          99, 
                            102,         108,        148
                            ], dtype=float)
...
class ContactResultsToArray(LeafSystem):
    def __init__(
        ...
        self.DeclarePeriodicDiscreteUpdateEvent(T, 0, self.Publish) # T used to be 0.0001
...
def quat_inverse(q):
    # assuming q is a numpy array [w, x, y, z] whhich is what drake is supposed to do
