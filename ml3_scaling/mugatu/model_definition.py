...
<robot name="walker">
  ...
  <link name="right_leg">
    ...
    <visual>
      ...
      <geometry>
        <mesh filename="file://{_MESH_DIR}/right_battery.obj" scale="{scale} {scale} {scale}"/>
      </geometry>
...
class Controller(LeafSystem):
    def __init__(
        ...
        if self.scale >= 6:
            ...
            hip_kp = hip_kp * 25000
            hip_kd = 13000
        elif 5 > self.scale >= 4.5:
            hip_kp = hip_kp * 12000
            hip_kd = 8000
        elif 4.5 > self.scale >= 3.2:
            hip_kp = hip_kp * 9000
            hip_kd = 1000
        elif 3.2 > self.scale >= 2:
            hip_kp = hip_kp * 3000
            hip_kd = 120
        elif 2 > self.scale >= 1.3:
            hip_kp = hip_kp * 1200
            hip_kd = 8
        elif 1.3 > self.scale >= 1:
            hip_kp = hip_kp * 200
            hip_kd = 2
        elif 1 > self.scale >= 0.6:
            hip_kp = hip_kp * 1 #hip_kp = 1 for scale = 0.166 (Zippy scale), hip_kp = 8000 for scale 6.67 (Big Foot scale)
            hip_kd = 0.05
        elif 0.6 > self.scale >= 0.3:
            hip_kp = hip_kp * 1 #hip_kp = 1 for scale = 0.166 (Zippy scale), hip_kp = 8000 for scale 6.67 (Big Foot scale)
        elif 0.3 > self.scale >= 0.2:
            hip_kp = hip_kp * 0.25
        elif 0.2 > self.scale >= 0.163:
            hip_kp = 0.8
        elif 0.163 > self.scale:
            hip_kp = 0.5
        else:
            hip_kd = 0
            hip_kp = 0.1
        ...
        self.frequency = 1.93 # works from 1.6 to 1.93
    ...
    def ComputeControl(self, current_state, desired_state, gain_matrix,feedforward=None):
        ...
        self.integral_error += (pos_error * self.control_period)
        self.derivative_error = vel_error

        feedback_input = self.hip_kp*pos_error + self.hip_ki * self.integral_error + self.hip_kd * self.derivative_error
    ...
    def Update(self, context, events):
        ...
        servo_vel = self.amplitude * ang_freq * np.cos(ang_freq * adjusted_time)
        if elapsed_time > act_start_time: # give time for sim to sit and settle
            ...
            self.target_state[14] = servo_vel # comment this out when you're getting sim natural frequency
        else:
           pass
